	...
	//if(event.keyCode == "13"){
		...
		if(arrayDept){
			$.each(arrayDept,function(k,v){
				...
				if(name===str){
					$("#show-add-organize-error").empty();
					$("#show-add-organize-error").append("该部门名称已存在空间下!");
...
$(document).on("click","#submitNewOrganizeInfo",function(){
	...
	if(type=='1'){
		...
		$("#show-add-organize-error").empty();
		$("#show-add-organize-error").append("部门名称为空或者已存在!");
		$("#show-add-organize-error").css("display","block");
		$("#submitNewOrganizeInfo").attr("status","0");
	}
});
organize.addNewOrganizeInfo = function(){
	...
	if(name){
		...
		if(status=='1'){
			startUp.postAsyncData(url,{"spaceId":spaceId,"name":name},function(data){
...
$(document).on("click",".delete-space-organize",function(){
	...
	if(organizeId){
		...
		startUp.postAsyncData(yz_url,{"id":organizeId},function(data){
			if(data.result>0){
				commonMethods.showConfirmNav("该分组下的成员将移到未分组中，是否确定要删除该组织分组?",function(){
					startUp.postAsyncData(url,{"id":organizeId,"spaceId":spaceId},function(data){
						organize.OneSapceOrganizesToList();
						if(status=='0'){
...
$(document).on("click",".remove-space-member",function(){
	...
	if(organizeId){
		commonMethods.showConfirmNav("是否确定要删除该成员?",function(){
			startUp.postAsyncData(url,{"id":organizeId,"userId":userId},function(data){
				if(data.result=='success'){
					if(status=='0'){
						$("#show-department-name").empty();
						$("#show-department-name").append("所有成员");
...
$(document).on("change",".to-update-organize",function(){
	...
	if(organizeId){
		commonMethods.showConfirmNav("是否确定要移动该成员?",function(){
			startUp.postAsyncData(url,{"id":organizeId,"userId":userId,"newOrganizeId":newOrganizeId},function(data){
				if(status=='0'){
					$("#show-department-name").empty();
					$("#show-department-name").append("所有成员");
					organize.showAllSpaceMember();
...
$(document).on("click",".space-department-each",function(){
	...
	startUp.postAsyncData(url,{"id":organizeId},function(data){
		$("#show-all-space-member").empty();
		$("#show-department-member-count").empty();
		$("#show-department-member-count").append(data.result.length);
		if(data.result.length>0){
...
$(document).on("click",".show-this-organize-member",function(){
	...
	if(organizeId){
		startUp.postAsyncData(url,{"id":organizeId},function(data){
			...
			if(data.result.length>0){
				$.each(data.result,function(k,v){
					htmlStr += organizeView.splitOrganizeMemberCss(null,v);
				});
			}
...
$(document).on("click","#submit-to-edit-oragnize",function(){
	...
	if(name){
		if(name.length>15){return;}
		startUp.postAsyncData(url,{"id":id,"name":name,"userId":userId,"spaceId":spaceId},function(data){
