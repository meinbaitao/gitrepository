		...
		<form id="loginFormost" class="" action="../mytask/mytask.html" method="post" onkeydown="if(event.keyCode==13)return false;">
		  <!-- 请输入邮箱/用户名  -->
			  <div class="user-pword"> 
				...
				<div class="input_div">
			  		<input type="text" id="username" placeholder="请输入邮箱/用户名" name="username" autocomplete="off"/>
			  		<!-- 禁止(表单)用户名、密码自动填充 -->
		  ...
		  <!-- 请输入密码 -->
			  <div class="user-pword">
				...
				<div class="input_div">
			  		<input type="password" id="password" placeholder="请输入密码" name="password" value="" autocomplete="off"/>
			  	</div>
			  </div>
		  <!-- 提交表单-->
			  <div class="the_login">
	...
	$(document).on('touchstart','.the_login input',function(){
		...
		if ( username == ''|| password == ''){
			...
			startUp.ajaxForm(url, data, false, function(resultMap){
				...
				var user_id = 'user_id';
				$.cookie(user_id, user_idd , { expires: 30, path: '/' });
				$.cookie(COOKIE_NAME, "1a072407d9e14c11b3a2fb50eb0bb2af", { expires: 30, path: '/' });
					if(resultMap){
						if(resultMap.status == "1"){
							$.cookie("status", "100", { expires: 30, path: '/' });
